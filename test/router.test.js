...
test('Skill Bundle Integrity Check', async (t) => {
    ...
    await t.test('Version Sync', () => {
        ...
        assert.ok(content.includes(`skill_version: ${pkg.version}`), `YAML frontmatter should match ${pkg.version}`);
        assert.ok(content.includes(`version: ${pkg.version}`), `Skill metadata should match ${pkg.version}`);
    });
    ...
    await t.test('Module Bundling', () => {
        ...
        assert.ok(content.includes('### MODULE: Technical Module'), 'Technical Module should be bundled');
        assert.ok(content.includes('### MODULE: Academic Module'), 'Academic Module should be bundled');
        assert.ok(content.includes('### MODULE: Governance Module'), 'Governance Module should be bundled');
    });
